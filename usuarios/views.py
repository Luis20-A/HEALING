...
def cadastro(request):
    if request.method == "GET":
        ...
        if len(senha) < 6 or not any(char.isdigit() for char in senha) or not any(char.isalpha() for char in senha):
            messages.add_message(request, constants.ERROR, "A senha deve ter 6 dígitos, conter letras, números e símbolos")
            return redirect ("/usuarios/cadastro")
