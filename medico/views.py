...
def abrir_horario(request):
    ...
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Área exclusiva apenas para medicos')
    ...
    if request.method == "GET":
        ...
        if data_formatada <= datetime.now():
            messages.add_message(request, constants.WARNING, 'A data da marcação não pode ser inferior a data atual')
            return redirect('/medicos/abrir_horario')
...
def add_documento(request, id_consulta):
    ...
    if consulta.status == 'A':
        messages.add_message(request, constants.WARNING, 'Só pode enviar um documento após iniciar a consulta!')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
    ...
    documento.save()
    messages.add_message(request, constants.SUCCESS, 'Documento enviado!')
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
